...
def get_supabase_client() -> Client:
    ...
    # Restore session if tokens stored in session_state
...
def get_current_user_role() -> str:
    return st.session_state.get("sb_role", "analista")


def is_editor() -> bool:
    """Supervisor o Administrador pueden editar estados."""
    return get_current_user_role() in ("supervisor", "administrador")


def require_auth():
    """Llama esto al inicio de cada página para protegerla."""
...
def _load_profile(client: Client, user_id: str):
    try:
        ...
        if res.data:
            ...
            st.session_state["sb_role"]      = res.data.get("role", "analista")
    except Exception:
        st.session_state["sb_role"] = "analista"
