...
from app.components import render_sidebar, kpi_card, progress_bar_html, semaforo_html, render_page_header
from app.database import get_kpis, get_phase_progress, get_project_config
from app.utils import format_date_es, get_current_week, get_end_date
...
for fn, pd_data in phase_progress.items():
    ...
    emoji, label = _get_semaforo(pd_data)
...
# helper local
def _get_semaforo(ph):
    from app.utils import semaforo_fase
    return semaforo_fase(ph)
...
with chart_col1:
    ...
    fig_donut.update_layout(
        ...
        plot_bgcolor="white",
        showlegend=True,
        legend=dict(orientation="h", y=-0.05, x=0.5, xanchor="center", font=dict(size=12, family="DM Sans")),
        margin=dict(t=10, b=10, l=10, r=10),
        height=260,
    )
    st.plotly_chart(fig_donut, use_container_width=True, config={"displayModeBar": False})

# Bar chart — Actividades por fase
with chart_col2:
